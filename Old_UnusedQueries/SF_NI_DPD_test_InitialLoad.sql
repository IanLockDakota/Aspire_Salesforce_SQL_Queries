...
USING (SELECT
    NULL AS ID,
    mainCon.ContractOid AS ContractOID__c,
    ...
    ISNULL(CASE WHEN CurrentDPD.DPD IS NULL AND mainCon.IsTerminated = 0 THEN 0 ELSE CurrentDPD.DPD END,'') AS DPD__c,
    ISNULL(NITerm.[Net Investment],'') AS NI_at_Termination__c,
    ISNULL(CASE WHEN DPDatTerm.DPD IS NULL AND mainCon.IsTerminated = 1 THEN 0 ELSE DPDatTerm.DPD END,'') AS DPD_at_Termination__c,
    ISNULL(NITerm.[Payments Made at Term],0) AS Payments_Made_At_Termination__c,
    ISNULL(NITerm.[Balance Remaining at Term]*-1,0) AS Balance_Remaining_At_Termination__c

FROM            
    [ASPIRESQL].[AspireDakotaTest].[dbo].[Contract] mainCON
LEFT OUTER JOIN
...
/* NetInvestment */
    ...
    (SELECT GLA.ContractOid,
        ...
        GROUP BY GLA.ContractOid) AS NIValue ON mainCon.ContractOid = NIValue.ContractOid 
...
(SELECT
	...
	WHERE (c2.IsBooked = 1) AND (c2.CompanyOid = 1) AND (c2.IsTerminated = 1)) AS NITerm ON NITerm.ContractOid = mainCon.ContractOid 
        ...
        FROM            
            ( SELECT        
                ...
                FROM [ASPIRESQL].[AspireDakotaTest].[dbo].[Contract] contr LEFT OUTER JOIN
                    ...
                    [ASPIRESQL].[AspireDakotaTest].[dbo].[Contract] C ON c.contractOID = OldRentDueDtTERM.ContractOid) AS CurrentDPD ON mainCon.ContractOid = CurrentDPD.ContractOid 
...
/* Days past due at term */
    ...
    (SELECT        
        ...
        FROM            
            ( SELECT        
                ...
                FROM [ASPIRESQL].[AspireDakotaTest].[dbo].[Contract] con LEFT OUTER JOIN
                    ...
                    [ASPIRESQL].[AspireDakotaTest].[dbo].[Contract] C ON c.contractOID = OldRentDueDtTERM.ContractOid) AS DPDatTerm ON mainCon.ContractOID = DPDatTerm.contractOID INNER JOIN
        ...
        WHERE GF.oid = 23
        GROUP BY c.ContractOID, GV.ref_oid, GF.descr, GV.field_value) AS OppIDTable ON mainCon.ContractOid = OppIDTable.ContractOID

WHERE 
    (mainCon.IsBooked = 1) AND (mainCon.CompanyOid = 1)
ORDER BY 
    mainCon.ContractOid) AS Source
