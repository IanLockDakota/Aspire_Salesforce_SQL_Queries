...
WHERE
    (Contract.isTerminated = 0) AND (Contract.CompanyOid = 1) AND (Contract.IsBooked = 1) AND (InvoicedPymnts.LastChangeDateTime BETWEEN @start and @end) AND (OppIDTable.opportunityID IS NOT NULL)) AS Source
