...
WHERE (C.CompanyOID = 1) AND (C.IsBooked = 1) AND (NOT (EFTS.EftScheduleOid IS NULL)) AND (NOT EFTB.description LIKE '%ATA%') AND (EFTS.LastChangeDateTime BETWEEN @start AND @end) AND (OppIDTable.opportunityID IS NOT NULL)) AS Source
