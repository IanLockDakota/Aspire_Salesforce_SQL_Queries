...
WHERE        (Contract.isTerminated = 0) AND (Contract.IsBooked = 1) AND (Contract.CompanyOid = 1) AND (OppIDTable.OppurtunityID IS NOT NULL) AND (AchBankAccount.AccountNumber Is NOT NULL) AND (ContractEFT.LastChangeDateTime BETWEEN @start AND @end)) AS Source
