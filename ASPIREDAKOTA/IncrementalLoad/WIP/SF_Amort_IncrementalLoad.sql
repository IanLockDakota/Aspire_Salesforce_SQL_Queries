	...
	LEFT OUTER JOIN
		...
		FROM
			...
			LEFT OUTER JOIN
				...
				WHERE
					p.IsReturned = 0) as p ON i.InvoiceDetailOid = p.InvoiceDetailOid) AS pay ON amort.ContractOid = pay.ContractOid AND amort.Date = pay.InvoiceDueDate AND amort.Payment = pay.TotalAppliedAmount
WHERE
	(amort.LCDT BETWEEN @start AND @end)) AS Source ON target.LineItemOID__c = source.LineItemOID__c
