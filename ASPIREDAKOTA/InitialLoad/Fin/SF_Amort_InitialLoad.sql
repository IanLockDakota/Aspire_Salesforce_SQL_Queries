...
USING (SELECT DISTINCT
	...
	LEFT OUTER JOIN
		(SELECT c.ContractOID, GV.ref_oid, GF.descr, ISNULL((GV.field_value), 'NULL') AS opportunityID
			...
			WHERE GF.oid = 23
			GROUP BY c.ContractOID, GV.ref_oid, GF.descr, GV.field_value) AS OppID ON c.contractOID = OppID.ContractOID
	WHERE 
		(C.IsBooked = 1) AND (C.CompanyOid = 1) AND (S.RefType = 1) AND (OppID.opportunityID IS NOT NULL)) AS Source
