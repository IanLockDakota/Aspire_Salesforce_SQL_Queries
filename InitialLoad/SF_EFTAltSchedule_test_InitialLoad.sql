...
FROM            [ASPIRESQL].[AspireDakotaTest].[dbo].[AchBankAccount] ABA INNER JOIN
                         [ASPIRESQL].[AspireDakotaTest].[dbo].[EftSchedule] EFTS ON ABA.AchBankAccountOid = EFTS.ReceivingBankAccountOid INNER JOIN
                         [ASPIRESQL].[AspireDakotaTest].[dbo].[AchBank] AB ON ABA.AchBankOid = AB.AchBankOid RIGHT OUTER JOIN
                         [ASPIRESQL].[AspireDakotaTest].[dbo].[Contract] C ON EFTS.ContractOid = C.ContractOid LEFT OUTER JOIN
						 [ASPIRESQL].[AspireDakotaTest].[dbo].[EFTBank] EFTB ON EFTS.OriginatingBankOid = EFTB.EFTBankOID LEFT OUTER JOIN
...
WHERE (C.CompanyOID = 1) AND (C.IsBooked = 1) AND (NOT (EFTS.EftScheduleOid IS NULL)) AND (NOT EFTB.description LIKE '%ATA%')) AS Source
