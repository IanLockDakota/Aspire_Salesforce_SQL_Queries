...
FROM            [ASPIRESQL].[AspireDakotaTest].[dbo].[AchBankAccount] INNER JOIN
                         [ASPIRESQL].[AspireDakotaTest].[dbo].[ContractEFT] ON AchBankAccount.AchBankAccountOid = ContractEFT.ACHBankAccountOid INNER JOIN
                         [ASPIRESQL].[AspireDakotaTest].[dbo].[AchBank] ON AchBankAccount.AchBankOid = AchBank.AchBankOid LEFT OUTER JOIN
                         [ASPIRESQL].[AspireDakotaTest].[dbo].[Contract] ON ContractEFT.ContractOid = Contract.ContractOid LEFT OUTER JOIN
...
WHERE        (Contract.IsBooked = 1) AND (Contract.CompanyOid = 1) AND (Contract.contractOID IN ('13582', '13563', '13643'))) AS Source
